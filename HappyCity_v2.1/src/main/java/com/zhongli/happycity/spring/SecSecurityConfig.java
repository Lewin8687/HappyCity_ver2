...
public class SecSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		// .authorizeRequests()
		// .antMatchers( "/j_spring_security_check*", "/login*", "/logout*",
		// "/signin/**", "/signup/**",
		// "/user/registration*", "/regitrationConfirm*", "/expiredAccount*",
		// "/registration*",
		...
		// should be submitted
		// // default-target-url: the URL to which the user will be redirected
		// if they login successfully
		...
		// // the user to in order to logout, the logout-success-url is where
		// they are taken if the logout
		...
		http
				// access-denied-page: this is the page users will be
				// redirected to when they try to access protected areas.
				.exceptionHandling().accessDeniedPage("/403").and().csrf().disable().authorizeRequests()
				.antMatchers("/j_spring_security_check*", "/login*", "/logout*", "/signin/**", "/signup/**",
						...
						"/user/changePassword*", "/emailError*", "/resources/**", "/successRegister*", "/", "/index*")
				.permitAll().antMatchers("/invalidSession*").anonymous().anyRequest().authenticated()
				.antMatchers("/sa/**").hasAuthority("ROLE_SUPPERADMIN").anyRequest().authenticated()
				.antMatchers("/admin/**", "/console*").hasAuthority("ROLE_ADMIN").anyRequest().authenticated()
				.antMatchers("/user/**").hasAuthority("ROLE_USER").anyRequest().authenticated().and().formLogin()
